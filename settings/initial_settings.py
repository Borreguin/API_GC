...
"""
    Esta API permite el despliegue de varios servicios web para la aplicación de 
    Manual Clasificador de Puestos. 
"""
...
if not DEBUG:
    ...
    host = config["MONGOCLIENT_SETTINGS"]['host']
    db_name = config["MONGOCLIENT_SETTINGS"]['db']
    port = config["MONGOCLIENT_SETTINGS"]['port']
    host_str = f'mongodb://{username}:{password}@{host}:{port}/{db_name}?authSource=admin'
    MONGOCLIENT_SETTINGS = dict(host=host_str)
...
TEMP_REPO, DB_REPO, LOG_REPO, EXCEL_REPO = FINAL_REPO
