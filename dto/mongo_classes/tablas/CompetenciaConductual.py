...
class CompetenciaConductual(Document):
    ...
    document = StringField(required=True, default="CompetenciaConductual")
    meta = {"collection": "TABLA|CompetenciaConductual"}
    ...
    def __repr__(self):
        return f"<CompetenciaConductual {self.idx}, {self.denominacion_competencia}, {self.item}, {self.nivel}>"
...
class CompetenciaConductualAsDataFrame:
    def __init__(self, excel_path=None):
        """
        This class allows to get CompetenciaConductual's DataFrame from Excel or Mongo DataBase
        :param excel_path: If one gets a dataFrame from excel
        """
        self.excel_path = excel_path
        self.sheet_name = "competencias_conductuales"
    ...
    def get_object_list_from_excel(self):
        # gets a list of CompetenciaConductual's objects
        ...
        try:
            ...
            for idx in self.df.index:
                ...
                item = CompetenciaConductual(**param)
                lst.append(item)
            return True, lst, "Lista de objetos creada de manera correcta"
        except Exception as e:
            return False, list(), f"Ha ocurrido un error al generar los objectos tipo CompetenciaConductual: {str(e)}"

    def get_excel_from_db(self, path_file: str, columns: list = None):
        items_as_dict = [it.to_dict() for it in CompetenciaConductual.objects.all()]
