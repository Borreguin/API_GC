...
class MisionAsDataFrame:
    def __init__(self, excel_path=None):
        """
        This class allows to get Mision's DataFrame from Excel or Mongo DataBase
        :param excel_path: If one gets a dataFrame from excel
        """
        self.excel_path = excel_path
        self.sheet_name = "mision"
        ...
        self.main_columns = [self.cl_id, self.cl_nombre, self.descripcion]
    ...
    def validate(self):
        ...
        success, self.df, msg = u.check_string_in_df(self.df, [self.cl_id, self.cl_nombre])
        if not success:
            return False, msg
    ...
    def get_object_list_from_excel(self):
        # gets a list of Mision's objects
        ...
        try:
            ...
            return False, list(), f"Ha ocurrido un error al generar los objectos tipo Mision: {str(e)}"

    def get_excel_from_db(self, path_file: str, columns: list = None):
        items_as_dict = [it.to_dict() for it in Mision.objects.all()]
