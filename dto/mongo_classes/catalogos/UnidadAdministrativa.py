...
class UnidadAdministrativa(Document):
    ...
    def to_dict(self):
        return dict(public_id=self.public_id, idx=self.idx, nombre=self.nombre,
                    cod_unidad_administrativa=self.cod_unidad_administrativa,
                    updated=self.updated.strftime(init.DEFAULT_DATE_FORMAT),
                    document=self.document)

    def to_update(self):
        return dict(idx=self.idx, nombre=self.nombre,
                    cod_unidad_administrativa=self.cod_unidad_administrativa,
                    updated=self.updated.strftime(init.DEFAULT_DATE_FORMAT),
                    document=self.document)


class UnidadAdministrativaAsDataFrame:
    def __init__(self, excel_path=None):
        """
        This class allows to get UnidadAdministrativa's DataFrame from Excel or Mongo DataBase
        :param excel_path: If one gets a dataFrame from excel
        """
        self.excel_path = excel_path
        self.sheet_name = "unidad_administrativa"
        # columns for this DataFrame
        self.cl_id = "idx"
        self.cl_unidad_administrativa = "cod_unidad_administrativa"
        self.cl_nombre = "nombre"
        self.df = pd.DataFrame()
        # columns to check in the excel file
        self.main_columns = [self.cl_id, self.cl_unidad_administrativa, self.cl_nombre]
    ...
    def validate(self):
        ...
        success, self.df, msg = u.check_string_in_df(self.df, [self.cl_id, self.cl_unidad_administrativa])
        if not success:
            return False, msg
    ...
    def get_object_list_from_excel(self):
        # gets a list of UnidadAdministrativa's objects
        ...
        try:
            ...
            for idx in self.df.index:
                ...
                item = UnidadAdministrativa(**param)
                lst.append(item)
            return True, lst, "Lista de objetos creada de manera correcta"
        except Exception as e:
            return False, list(), f"Ha ocurrido un error al generar los objectos tipo UnidadAdministrativa: {str(e)}"

    def get_excel_from_db(self, path_file: str, columns: list = None):
        items_as_dict = [it.to_dict() for it in UnidadAdministrativa.objects.all()]
