...
class Ambito(Document):
    ...
    def to_update(self):
        return dict(idx=self.idx, ambito=self.ambito,
                    updated=self.updated.strftime(init.DEFAULT_DATE_FORMAT),
                    document=self.document)


class AmbitoAsDataFrame:
    def __init__(self, excel_path=None):
        """
        This class allows to get Ambito's DataFrame from Excel or Mongo DataBase
        ...
        self.main_columns = [self.cl_id, self.cl_ambito]
    ...
    def get_excel_from_db(self, path_file: str, columns: list = None):
        items_as_dict = [it.to_dict() for it in Ambito.objects.all()]
