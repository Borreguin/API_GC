...
class InstitucionExterna(Document):
    ...
    document = StringField(required=True, default="InstitucionExterna")
    meta = {"collection": "CATALOGO|InstitucionExterna"}
    ...
    def __repr__(self):
        return f"<InstitucionExterna {self.codigo},{self.nombre}>"
    ...
    def to_update(self):
        return dict(idx=self.idx, codigo=self.codigo,
                    nombre=self.nombre,
                    updated=self.updated.strftime(init.DEFAULT_DATE_FORMAT),
                    document=self.document)


class InstitucionExternaAsDataFrame:
    def __init__(self, excel_path=None):
        """
        This class allows to get InstitucionExterna's DataFrame from Excel or Mongo DataBase
        :param excel_path: If one gets a dataFrame from excel
        """
        self.excel_path = excel_path
        self.sheet_name = "instituciones_externas"
        ...
        self.main_columns = [self.cl_id, self.cl_codigo, self.nombre]
    ...
    def get_object_list_from_excel(self):
        # gets a list of InstitucionExterna's objects
        ...
        try:
            ...
            for idx in self.df.index:
                ...
                item = InstitucionExterna(**param)
                lst.append(item)
            return True, lst, "Lista de objetos creada de manera correcta"
        except Exception as e:
            return False, list(), f"Ha ocurrido un error al generar los objectos tipo InstitucionExterna: {str(e)}"

    def get_excel_from_db(self, path_file: str, columns: list = None):
        items_as_dict = [it.to_dict() for it in InstitucionExterna.objects.all()]
